...
[~, maxFreqIndices] = max(S, [], 1);  % Find index of maximum magnitude for each time point
dominantFreqs = f(maxFreqIndices);     % Convert indices to frequencies

% Plot dominant frequencies
subplot(2,1,2);
plot(t, dominantFreqs, 'b-', 'LineWidth', 1.5);
title('Dominant Frequencies Over Time');
xlabel('Time (s)');
ylabel('Frequency (Hz)');
grid on;
ylim([0, Fs/2]);  % Set y-axis limits to match spectrogram

% Optional: Add threshold to ignore silent parts
threshold = max(max(S))/10;  % Adjust this value as needed
for i = 1:length(dominantFreqs)
    if max(S(:,i)) < threshold
        dominantFreqs(i) = NaN;  % Mark as NaN if below threshold
    end
end

% Plot thresholded dominant frequencies
figure;
