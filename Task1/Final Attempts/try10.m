...
dominant_freq_weight = 0.4;  % Weight for dominant frequency matching
spcc_weight = 0.3;           % Weight for spectrogram cross-correlation
tdcc_weight = 0.3;           % Weight for time-domain cross-correlation
...
if max_combined_score >= threshold
    ...
    method_used = 'Combined Dominant Frequencies, Spectrogram Cross-Correlation, and Time-Domain Cross-Correlation';
...
[overall_max_score, overall_stage_idx] = max([max_combined_score]);
if overall_stage_idx == 1
    best_match = ref_files{max_idx}; % Combined Stage
    method_used = 'Combined Stage (Fallback)';
end
