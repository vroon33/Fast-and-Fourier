...
% Plot a zoomed section to verify R-peak detection
figure;
zoom_start = 10 * Fs;  % Start at 10 seconds
zoom_duration = 5 * Fs;  % Show 5 seconds
zoom_range = zoom_start:(zoom_start + zoom_duration);

plot(t(zoom_range), E1_trimmed(zoom_range));
hold on;
zoom_peaks = R_locs(R_locs >= zoom_start & R_locs <= (zoom_start + zoom_duration));
plot(t(zoom_peaks), E1_trimmed(zoom_peaks), 'ro', 'MarkerSize', 8);
xlabel('Time (s)');
ylabel('ECG Amplitude');
title('Zoomed View of ECG with R-peak Detection');
grid on;