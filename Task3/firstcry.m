...
function filteredAudio = filter(audio,fs)
    ...
    filteredAudio = noisefilter(filteredAudio,fs);
    subplot(2,1,2);
...
%       'FilterOrder', 8, ...
%       'PassbandFrequency', 10000/Fn, ...    % Normalize by dividing by Fn
%       'PassbandRipple', 0.01);
  
%   % High-pass filter
%   hpFilt = designfilt('highpassiir', ...
%       'FilterOrder', 8, ...
%       'PassbandFrequency', 100 /Fn, ...   % Normalize by dividing by Fn
%       'PassbandRipple', 0.01);
  
  % Apply filters
    filteredAudio = noisefilter(audio,fs);
    % filteredAudio = filter(audio,fs);
    % pause(3);
    % sound(filteredAudio, fs);
    % figure;
